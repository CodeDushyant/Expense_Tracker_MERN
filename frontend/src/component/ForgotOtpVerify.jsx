...
const ForgotOtpVerify = () => {
  ...
  const handleVerifyOtp = async (data) => {
    try {
      ...
      const res = await axios.post("http://localhost:3000/api/v1/user/verifyOtpforResetPassword", {
        userId: userId,
        otp: data.otp
      });
  ...
  const handleResetPassword = async (data) => {
    try {
      const res = await axios.post("http://localhost:3000/api/v1/user/resetPassword", 
